        ...
        for k = 1:11
            for w = 1:11
                for q = 1:11
                    ...
                    costmat = zeros(11,11,11,12);

                    bestndt = -((squeeze(data.I_RT(curDataMask1,:,:,w,:)) - intercept_sub_xor.I_rt)/intercept_sub_xor.se_rt^2 + ...
                        (squeeze(data.I_RT(curDataMask2,:,:,q,:)) - intercept_sub_or.I_rt)/intercept_sub_or.se_rt^2)/(1/intercept_sub_xor.se_rt^2 + ...
                        1/intercept_sub_or.se_rt^2);

                    bestndt(bestndt<0) = 0;
                    costmat(:,:,:,1) = ((squeeze(data.b1_RT(curDataMask1,:,:,w,:)) - coeff_rt_sub_xor.beta_OV)/coeff_rt_sub_xor.se_OV).^2;
                    costmat(:,:,:,2) = ((squeeze(data.b2_RT(curDataMask1,:,:,w,:)) - coeff_rt_sub_xor.beta_VD)/coeff_rt_sub_xor.se_VD).^2;
                    costmat(:,:,:,3) = ((squeeze(data.b1_Acc(curDataMask1,:,:,w,:)) - coeff_acc_sub_xor.beta_OV)/coeff_acc_sub_xor.se_OV).^2;
                    costmat(:,:,:,4) = ((squeeze(data.b2_Acc(curDataMask1,:,:,w,:)) - coeff_acc_sub_xor.beta_VD)/coeff_acc_sub_xor.se_VD).^2;
                    costmat(:,:,:,5) = ((bestndt + squeeze(data.I_RT(curDataMask1,:,:,w,:)) - intercept_sub_xor.I_rt)/intercept_sub_xor.se_rt).^2;
                    costmat(:,:,:,6) = ((squeeze(data.I_Acc(curDataMask1,:,:,w,:)) - intercept_sub_xor.I_acc)/intercept_sub_xor.se_acc).^2;
                    costmat(:,:,:,7) = ((squeeze(data.b1_RT(curDataMask2,:,:,q,:)) - coeff_rt_sub_or.beta_OV)/coeff_rt_sub_or.se_OV).^2;
                    costmat(:,:,:,8) = ((squeeze(data.b2_RT(curDataMask2,:,:,q,:)) - coeff_rt_sub_or.beta_VD)/coeff_rt_sub_or.se_VD).^2;
                    costmat(:,:,:,9) = ((squeeze(data.b1_Acc(curDataMask2,:,:,q,:)) - coeff_acc_sub_or.beta_OV)/coeff_acc_sub_or.se_OV).^2;
                    costmat(:,:,:,10) = ((squeeze(data.b2_Acc(curDataMask2,:,:,q,:)) - coeff_acc_sub_or.beta_VD)/coeff_acc_sub_or.se_VD).^2;
                    costmat(:,:,:,11) = ((bestndt + squeeze(data.I_RT(curDataMask2,:,:,q,:)) - intercept_sub_or.I_rt)/intercept_sub_or.se_rt).^2;
                    costmat(:,:,:,12) = ((squeeze(data.I_Acc(curDataMask2,:,:,q,:)) - intercept_sub_or.I_acc)/intercept_sub_or.se_acc).^2;
                    bestndtmat(:,:,:) = bestndt;
                    ...
                    if min(cost(:)) < mincost
                        ...
                        indices = [w,q];
                        for j = 1:2
                            decays(j) = data.decay(1,ii,jj,indices(j),kk);
                            threshs(j) = data.thresh(1,ii,jj,indices(j),kk);
                            gains(j) = data.gain(1,ii,jj,indices(j),kk);
                            mis(j) = data.mutualInhib(1,ii,jj,indices(j),kk);
                            orders(j) = order;
                            nondecsT(j) = bestndtmat(ii,jj,kk);
                        end
                        nDTs(1) = nDTlist(m);
                        nDTs(2) = nDTlist(k);
                    end
...
writetable(table_outcome,'../result/prediction_task_12_bound.csv');