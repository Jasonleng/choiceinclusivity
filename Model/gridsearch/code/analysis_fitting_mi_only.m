...
%             intercept_sub = intercept(strcmp(intercept.study,study) & strcmp(intercept.catChoiceCondition,conds{j}),:);
    ...
    if min(cost(:)) < mincost
        ...
        for m = 1:2
            decays(m) = data.decay(j,ii,jj,kk);
            threshs(m) = data.thresh(j,ii,jj,kk);
            gains(m) = data.gain(j,ii,jj,kk);
            orders(m) = data.order;
            nDTs(m) = data.nDT;
            nondecsT(m) = bestndtmat(ii,jj,kk,ww,pp);
        end
        
        mis(1) = data.mutualInhib(ww,ii,jj,kk);
        mis(2) = data.mutualInhib(pp,ii,jj,kk);
        file = files(i).name;
    end
end
...
% besttheta = [gains(1) 0 decays(1)-1 decays(1)-1 threshs(1) nondecsT(1) 0.01 0.5];
% decay = decay - 0.5;
% outcome = summaryPrediction(besttheta,nDTs(1),orders(1),[0.75,0.25]);

besttheta = [gains(1) 0 decays(1) (decays(1))*mis(1) threshs(1) nondecsT(1) 0 0.5;
             gains(1) 0 decays(1) (decays(1))*mis(2) threshs(1) nondecsT(1) 0 0.5];
...
writetable(table_outcome,'../result/prediction_task_12_inhib.csv');

save("../result/params_inhib.mat","besttheta","nDTs","orders");