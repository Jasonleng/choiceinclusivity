...
df.total.rt.cfl.6$scaledRT<-scale(log10(df.total.rt.cfl.6$firstRT))
df.total.rt.cfl.6$catAcc<-factor(df.total.rt.cfl.6$isBestFirst)
contrasts(df.total.rt.cfl.6$catAcc)<-contr.sdif

model.rating.cfl.choice <- lmerTest::lmer(conflict ~ scaledRT + catAcc + catChoiceCondition * (poly(scaledMeanValue,2) + scaledValueDiff) + scaledTrialNum + (1 + scaledRT + catAcc + catChoiceCondition * (poly(scaledMeanValue,2) + scaledValueDiff) + scaledTrialNum|participant),data=df.total.rt.cfl.6,control=lmerControl(optCtrl=list(maxfun=100000)))
